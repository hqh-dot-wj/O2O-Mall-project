...
---
description: Admin-Web 前端开发规范（目录结构、命名、布局、性能、CSS、TypeScript 类型安全、测试）
...
- **API 类型必须来自 `@libs/common-types`**（由 backend openApi.json 通过 `pnpm generate-types` 生成），**禁止手写**与后端 VO/DTO 重复的类型。
- 开发涉及 API 的页面**前**：先执行 `pnpm generate-types`，再从 `@libs/common-types` 或 typings 中引用；typings 文件应通过 `import('@libs/common-types')` 引用，而非手写接口定义。
- 详见规则：`admin-web-api-types.mdc`。

### 9.3 Props / Emits 显式类型
...
- **单元/组件测试**：Vitest + `@vue/test-utils`，用例放在 `src/**/*.spec.{ts,tsx,vue}`，配置在 `vitest.config.ts`、入口 `src/test/setup.ts`。
- **E2E 测试**：Playwright，用例放在 `e2e/*.spec.ts`，配置在 `playwright.config.ts`。
