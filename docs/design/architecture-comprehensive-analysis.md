...
## 附录 E: 平台功能完整性评估 — 缺失模块清单

> **更新日期**: 2026-02-24  
> **评估基准**: O2O 家政/上门服务平台完整业务闭环  
> **对标竞品**: 58到家/天鹅到家、美团到家、好慷在家

### E.1 现有模块总览

| 模块      | 目录                | 状态      | 说明                                             |
| --------- | ------------------- | --------- | ------------------------------------------------ |
| admin     | `module/admin/`     | ✅ 完整   | 后台管理 (用户/角色/权限/监控/资源)              |
| client    | `module/client/`    | ✅ 完整   | C端消费者 (认证/商品/购物车/订单/支付/地址/营销) |
| worker    | `module/worker/`    | ❌ 空目录 | 劳动者端 — 无任何代码                            |
| store     | `module/store/`     | ✅ 基础   | 门店管理 (订单/商品/分销/库存/财务)              |
| finance   | `module/finance/`   | ✅ 基础   | 佣金/钱包/提现/结算                              |
| marketing | `module/marketing/` | ✅ 较完整 | 积分/优惠券/营销活动/审批/灰度                   |
| pms       | `module/pms/`       | ✅ 基础   | 商品标准库 (分类/品牌/属性/商品)                 |
| lbs       | `module/lbs/`       | ✅ 基础   | 位置服务 (区域/站点/围栏)                        |
| risk      | `module/risk/`      | ⚠️ 骨架   | 仅 service + module，无具体实现                  |
| common    | `module/common/`    | ✅ 基础   | Redis/Axios/Bull 队列                            |
| backup    | `module/backup/`    | ✅ 完整   | 数据备份                                         |
| main      | `module/main/`      | ✅ 完整   | 健康检查                                         |

### E.2 缺失模块清单

| #   | 缺失模块                           | 优先级 | 说明                                                           |
| --- | ---------------------------------- | ------ | -------------------------------------------------------------- |
| 1   | **Worker 模块** (劳动者端)         | P0     | 接单/排班/收入/核销/个人资料，目录已建但完全为空               |
| 2   | **多端认证模块**                   | P0     | 当前仅支持微信小程序，缺 H5 OAuth、短信验证码、App 登录策略    |
| 3   | **消息/通知模块** (Notification)   | P1     | 缺统一消息中心：站内信、微信模板消息、短信通知、App 推送       |
| 4   | **支付模块** (Payment Core)        | P1     | client/payment 存在但缺微信 JSAPI 支付(H5)、App 支付、退款回调 |
| 5   | **评价模块** (Review)              | P1     | 无用户评价/评分体系，竞品均有完整的服务评价 + 技师评分         |
| 6   | **调度/派单引擎** (Dispatch)       | P2     | 当前仅手动派单，缺基于距离+评分+负载的智能匹配                 |
| 7   | **售后模块** (AfterSale)           | P2     | 无独立售后流程，退款逻辑嵌在 store-order 中，缺投诉/纠纷/仲裁  |
| 8   | **数据统计/报表模块** (Analytics)  | P2     | 无经营数据看板、订单统计、收入报表、用户分析                   |
| 9   | **内容/CMS 模块** (Content)        | P3     | 无公告、帮助中心、服务协议、Banner 管理                        |
| 10  | **客服/IM 模块** (CustomerService) | P3     | 无在线客服、工单系统，竞品均有 IM 或工单                       |

### E.3 现有模块功能缺口

| 模块    | 缺口                                   | 优先级 |
| ------- | -------------------------------------- | ------ |
| risk    | 仅骨架，无黑名单/风控规则/异常检测实现 | P1     |
| finance | 缺发票、充值、对账、财务报表           | P2     |
| store   | 缺门店设置、营业时间、服务区域配置     | P2     |
| lbs     | 缺实时位置上报、轨迹记录、距离计算服务 | P2     |
| pms     | 缺商品导入/导出、批量操作              | P3     |
...
> 详细设计方案见 [多角色多端架构设计方案](./multi-role-multi-client-architecture-plan.md)
