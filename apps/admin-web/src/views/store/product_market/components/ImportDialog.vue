...
<template>
  <NModal v-model:show="visible" preset="dialog" title="确认上架" style="width: 800px">
    <div v-if="product" class="flex flex-col gap-4 py-4">
      <!-- Header Info -->
      <div class="flex items-center gap-4 rounded bg-gray-50 p-4 dark:bg-gray-800">
        <NImage
      ...
      <NForm label-placement="left" label-width="120">
        ...
        <NFormItem v-if="product.type === 'SERVICE'" label="服务半径 (米)">
          <NInputNumber v-model:value="formModel.overrideRadius" :min="0" placeholder="默认使用总部配置">
            <template #suffix>米</template>
          </NInputNumber>
          <template #feedback>留空则使用总部默认半径: {{ product.serviceRadius || '未设置' }}m</template>
        </NFormItem>
