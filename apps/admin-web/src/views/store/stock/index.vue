...
import { NButton, NSpace, NAvatar, NDataTable, NCard, NInput, NInputNumber, NPopover } from 'naive-ui';
...
const StockUpdatePopover = (props: { row: Api.Store.StockSku; type: 'add' | 'reduce'; onSuccess: () => void }) => {
    ...
    return (
        <NPopover trigger="click" placement="bottom">
            {{
                trigger: () => <NButton size="small" type={btnType} secondary>{btnText}</NButton>,
                default: () => (
                    <NSpace vertical>
    ...
    <NCard title="Stock Management" :bordered="false" class="h-full rounded-8px shadow-sm">
      <div class="h-full flex-col-stretch gap-12px">
        <NSpace justify="space-between">
           <NSpace>
              <NInput v-model:value="searchParams.productName" placeholder="Search Product Name" clearable @keydown.enter="getData" />
