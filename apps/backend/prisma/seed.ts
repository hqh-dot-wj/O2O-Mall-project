/**
 * Prisma Seed Data
 *
 * 包含：租户、客户端、字典、配置、部门、岗位、角色、菜单、用户及关联关系。
 * 已补充：订单派单、会员管理、营销管理（优惠券、积分、课程等）菜单。
 *
 * 运行方式: pnpm prisma:seed（完整重置+种子） 或 pnpm prisma:seed:only（仅种子，不重置）
 */

import { PrismaClient } from '@prisma/client';

const prisma = new PrismaClient();

async function main() {
  console.log('开始导入种子数据...');

  // sys_tenant_package
  await prisma.sysTenantPackage.createMany({
    data: [
      {
        packageId: 1,
        packageName: '基础套餐',
        menuIds: '1,2,3,4,5,6,7,8,100,101,102,103,104,105,106,107,108,109,110,112,113,114,115,116,117,118,119,120,121,122,123,124,125,200,201,202,203,204,205,210,211,212,213,220,221,222,223,224,225,226,227,228,230,231,232,240,250,251,260,261,262,263,264,265,266,267,268,269,270,271,272,273,280,281,282,283,284,500,501,1000,1001,1002,1003,1004,1005,1006,1007,1008,1009,1010,1011,1012,1013,1014,1015,1016,1017,1018,1019,1020,1021,1022,1023,1024,1025,1026,1027,1028,1029,1030,1031,1032,1033,1034,1035,1036,1037,1038,1039,1040,1041,1042,1043,1044,1045,1046,1047,1048,1049,1050,1051,1052,1053,1054,1055,1056,1057,1058,1059,1060,1061,1062,1063,1064,1065,1066,1067,1068,1069,1070,1071,1072,1073,1074,1075,1076,1077,1078',
        menuCheckStrictly: true,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: new Date('2025-12-23 18:33:08.141308'),
        remark: '基础套餐，包含所有菜单权限',
      },
    ],
    skipDuplicates: true,
  });

  // sys_tenant
  await prisma.sysTenant.createMany({
    data: [
      {
        id: 1,
        tenantId: '000000',
        contactUserName: 'admin',
        contactPhone: '15888888888',
        companyName: '管理组',
        licenseNumber: null,
        address: null,
        intro: '系统默认租户，不可删除',
        domain: null,
        packageId: 1,
        expireTime: null,
        accountCount: -1,
        storageQuota: 10240,
        storageUsed: 0,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: new Date('2025-12-23 18:33:08.1474'),
        remark: '超级管理员租户',
        regionCode: null,
        isDirect: true,
      },
    ],
    skipDuplicates: true,
  });

  // sys_client
  await prisma.sysClient.createMany({
    data: [
      {
        id: 1,
        clientId: 'e5cd7e4891bf95d1d19206ce24a7b32e',
        clientKey: 'pc',
        clientSecret: 'pc123',
        grantTypeList: 'password,social',
        deviceType: 'pc',
        activeTimeout: 1800,
        timeout: 86400,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: new Date('2025-12-23 18:33:08.150524'),
      },
      {
        id: 2,
        clientId: '428a8310a3c9c5eb0a7c4c9d4b3d0c37',
        clientKey: 'app',
        clientSecret: 'app123',
        grantTypeList: 'password,social',
        deviceType: 'app',
        activeTimeout: 1800,
        timeout: 86400,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: new Date('2025-12-23 18:33:08.150524'),
      },
    ],
    skipDuplicates: true,
  });

  // sys_dict_type
  await prisma.sysDictType.createMany({
    data: [
      {
        dictId: 1,
        tenantId: '000000',
        dictName: '用户性别',
        dictType: 'sys_user_sex',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '用户性别列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 2,
        tenantId: '000000',
        dictName: '菜单状态',
        dictType: 'sys_show_hide',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '菜单状态列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 3,
        tenantId: '000000',
        dictName: '系统开关',
        dictType: 'sys_normal_disable',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统开关列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 4,
        tenantId: '000000',
        dictName: '任务状态',
        dictType: 'sys_job_status',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '任务状态列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 5,
        tenantId: '000000',
        dictName: '任务分组',
        dictType: 'sys_job_group',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '任务分组列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 6,
        tenantId: '000000',
        dictName: '系统是否',
        dictType: 'sys_yes_no',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统是否列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 7,
        tenantId: '000000',
        dictName: '通知类型',
        dictType: 'sys_notice_type',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '通知类型列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 8,
        tenantId: '000000',
        dictName: '通知状态',
        dictType: 'sys_notice_status',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '通知状态列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 9,
        tenantId: '000000',
        dictName: '操作类型',
        dictType: 'sys_oper_type',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '操作类型列表',
        delFlag: 'NORMAL',
      },
      {
        dictId: 10,
        tenantId: '000000',
        dictName: '系统状态',
        dictType: 'sys_common_status',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '登录状态列表',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_dict_data
  await prisma.sysDictData.createMany({
    data: [
      {
        dictCode: 1,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '男',
        dictValue: '0',
        dictType: 'sys_user_sex',
        cssClass: '',
        listClass: '',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '性别男',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 2,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '女',
        dictValue: '1',
        dictType: 'sys_user_sex',
        cssClass: '',
        listClass: '',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '性别女',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 3,
        tenantId: '000000',
        dictSort: 3,
        dictLabel: '未知',
        dictValue: '2',
        dictType: 'sys_user_sex',
        cssClass: '',
        listClass: '',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '性别未知',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 4,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '显示',
        dictValue: '0',
        dictType: 'sys_show_hide',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '显示菜单',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 5,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '隐藏',
        dictValue: '1',
        dictType: 'sys_show_hide',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '隐藏菜单',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 6,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '正常',
        dictValue: '0',
        dictType: 'sys_normal_disable',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '正常状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 7,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '停用',
        dictValue: '1',
        dictType: 'sys_normal_disable',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '停用状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 8,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '正常',
        dictValue: '0',
        dictType: 'sys_job_status',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '正常状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 9,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '暂停',
        dictValue: '1',
        dictType: 'sys_job_status',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '停用状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 10,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '默认',
        dictValue: 'DEFAULT',
        dictType: 'sys_job_group',
        cssClass: '',
        listClass: '',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '默认分组',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 11,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '系统',
        dictValue: 'SYSTEM',
        dictType: 'sys_job_group',
        cssClass: '',
        listClass: '',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统分组',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 12,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '是',
        dictValue: 'Y',
        dictType: 'sys_yes_no',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统默认是',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 13,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '否',
        dictValue: 'N',
        dictType: 'sys_yes_no',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统默认否',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 14,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '通知',
        dictValue: '1',
        dictType: 'sys_notice_type',
        cssClass: '',
        listClass: 'warning',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '通知',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 15,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '公告',
        dictValue: '2',
        dictType: 'sys_notice_type',
        cssClass: '',
        listClass: 'success',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '公告',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 16,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '正常',
        dictValue: '0',
        dictType: 'sys_notice_status',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'Y',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '正常状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 17,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '关闭',
        dictValue: '1',
        dictType: 'sys_notice_status',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '关闭状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 18,
        tenantId: '000000',
        dictSort: 99,
        dictLabel: '其他',
        dictValue: '0',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'info',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '其他操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 19,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '新增',
        dictValue: '1',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'info',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '新增操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 20,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '修改',
        dictValue: '2',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'info',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '修改操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 21,
        tenantId: '000000',
        dictSort: 3,
        dictLabel: '删除',
        dictValue: '3',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '删除操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 22,
        tenantId: '000000',
        dictSort: 4,
        dictLabel: '授权',
        dictValue: '4',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '授权操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 23,
        tenantId: '000000',
        dictSort: 5,
        dictLabel: '导出',
        dictValue: '5',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'warning',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '导出操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 24,
        tenantId: '000000',
        dictSort: 6,
        dictLabel: '导入',
        dictValue: '6',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'warning',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '导入操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 25,
        tenantId: '000000',
        dictSort: 7,
        dictLabel: '强退',
        dictValue: '7',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '强退操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 26,
        tenantId: '000000',
        dictSort: 8,
        dictLabel: '生成代码',
        dictValue: '8',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'warning',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '生成操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 27,
        tenantId: '000000',
        dictSort: 9,
        dictLabel: '清空数据',
        dictValue: '9',
        dictType: 'sys_oper_type',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '清空操作',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 28,
        tenantId: '000000',
        dictSort: 1,
        dictLabel: '成功',
        dictValue: '0',
        dictType: 'sys_common_status',
        cssClass: '',
        listClass: 'primary',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '正常状态',
        delFlag: 'NORMAL',
      },
      {
        dictCode: 29,
        tenantId: '000000',
        dictSort: 2,
        dictLabel: '失败',
        dictValue: '1',
        dictType: 'sys_common_status',
        cssClass: '',
        listClass: 'danger',
        isDefault: 'N',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '停用状态',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_config
  await prisma.sysConfig.createMany({
    data: [
      {
        configId: 1,
        tenantId: '000000',
        configName: '主框架页-默认皮肤样式名称',
        configKey: 'sys.index.skinName',
        configValue: 'skin-blue',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '蓝色 skin-blue、绿色 skin-green、紫色 skin-purple、红色 skin-red、黄色 skin-yellow',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 2,
        tenantId: '000000',
        configName: '用户管理-账号初始密码',
        configKey: 'sys.user.initPassword',
        configValue: '123456',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '初始化密码 123456',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 3,
        tenantId: '000000',
        configName: '主框架页-侧边栏主题',
        configKey: 'sys.index.sideTheme',
        configValue: 'theme-dark',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '深色主题theme-dark，浅色主题theme-light',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 4,
        tenantId: '000000',
        configName: '账号自助-验证码开关',
        configKey: 'sys.account.captchaEnabled',
        configValue: 'false',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '是否开启验证码功能（true开启，false关闭）',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 5,
        tenantId: '000000',
        configName: '账号自助-是否开启用户注册功能',
        configKey: 'sys.account.registerUser',
        configValue: 'false',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '是否开启注册用户功能（true开启，false关闭）',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 6,
        tenantId: '000000',
        configName: '用户登录-黑名单列表',
        configKey: 'sys.login.blackIPList',
        configValue: '',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '设置登录IP黑名单限制，多个匹配项以;分隔，支持匹配（*通配、网段）',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
      {
        configId: 7,
        tenantId: '000000',
        configName: '营销模块-平台费率',
        configKey: 'marketing.fee_rate',
        configValue: '0.01',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销实例成功后结算到门店钱包的平台费率，默认 1%',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_system_config
  await prisma.sysSystemConfig.createMany({
    data: [
      {
        configName: '营销模块-平台费率',
        configKey: 'marketing.fee_rate',
        configValue: '0.01',
        configType: 'Y',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销实例成功后结算到门店钱包的平台费率，默认 1%',
        status: 'NORMAL',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_dept
  await prisma.sysDept.createMany({
    data: [
      {
        deptId: 100,
        tenantId: '000000',
        parentId: 0,
        ancestors: '0',
        deptName: 'nest-admin-soybean科技',
        orderNum: 0,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 101,
        tenantId: '000000',
        parentId: 100,
        ancestors: '0,100',
        deptName: '深圳总公司',
        orderNum: 1,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 102,
        tenantId: '000000',
        parentId: 100,
        ancestors: '0,100',
        deptName: '长沙分公司',
        orderNum: 2,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 103,
        tenantId: '000000',
        parentId: 101,
        ancestors: '0,100,101',
        deptName: '研发部门',
        orderNum: 1,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 104,
        tenantId: '000000',
        parentId: 101,
        ancestors: '0,100,101',
        deptName: '市场部门',
        orderNum: 2,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 105,
        tenantId: '000000',
        parentId: 101,
        ancestors: '0,100,101',
        deptName: '测试部门',
        orderNum: 3,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 106,
        tenantId: '000000',
        parentId: 101,
        ancestors: '0,100,101',
        deptName: '财务部门',
        orderNum: 4,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 107,
        tenantId: '000000',
        parentId: 101,
        ancestors: '0,100,101',
        deptName: '运维部门',
        orderNum: 5,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 108,
        tenantId: '000000',
        parentId: 102,
        ancestors: '0,100,102',
        deptName: '市场部门',
        orderNum: 1,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
      {
        deptId: 109,
        tenantId: '000000',
        parentId: 102,
        ancestors: '0,100,102',
        deptName: '财务部门',
        orderNum: 2,
        leader: 'nest-admin-soybean',
        phone: '15888888888',
        email: 'demo@nestadmin.com',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: null,
      },
    ],
    skipDuplicates: true,
  });

  // sys_post
  await prisma.sysPost.createMany({
    data: [
      {
        postId: 1,
        tenantId: '000000',
        deptId: null,
        postCode: 'ceo',
        postCategory: null,
        postName: '董事长',
        postSort: 1,
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        postId: 2,
        tenantId: '000000',
        deptId: null,
        postCode: 'se',
        postCategory: null,
        postName: '项目经理',
        postSort: 2,
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        postId: 3,
        tenantId: '000000',
        deptId: null,
        postCode: 'hr',
        postCategory: null,
        postName: '人力资源',
        postSort: 3,
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        postId: 4,
        tenantId: '000000',
        deptId: null,
        postCode: 'user',
        postCategory: null,
        postName: '普通员工',
        postSort: 4,
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_role
  await prisma.sysRole.createMany({
    data: [
      {
        roleId: 1,
        tenantId: '000000',
        roleName: '超级管理员',
        roleKey: 'admin',
        roleSort: 1,
        dataScope: '1',
        menuCheckStrictly: false,
        deptCheckStrictly: false,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '超级管理员',
      },
      {
        roleId: 2,
        tenantId: '000000',
        roleName: '普通角色',
        roleKey: 'common',
        roleSort: 2,
        dataScope: '2',
        menuCheckStrictly: false,
        deptCheckStrictly: false,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '普通角色',
      },
      {
        roleId: 3,
        tenantId: '000000',
        roleName: '演示角色',
        roleKey: 'demo',
        roleSort: 10,
        dataScope: '5',
        menuCheckStrictly: false,
        deptCheckStrictly: false,
        status: 'NORMAL',
        delFlag: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '演示账户角色，仅拥有查看权限',
      },
    ],
    skipDuplicates: true,
  });

  // sys_menu
  await prisma.sysMenu.createMany({
    data: [
      {
        menuId: 1,
        tenantId: '000000',
        menuName: '系统管理',
        parentId: 0,
        orderNum: 1,
        path: 'system',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'system',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统管理目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 2,
        tenantId: '000000',
        menuName: '系统监控',
        parentId: 0,
        orderNum: 2,
        path: 'monitor',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'monitor',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统监控目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 3,
        tenantId: '000000',
        menuName: '系统工具',
        parentId: 0,
        orderNum: 3,
        path: 'tool',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'tool',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统工具目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 4,
        tenantId: '000000',
        menuName: 'nest-admin-soybean官网',
        parentId: 0,
        orderNum: 4,
        path: 'https://nest-admin.dooring.vip',
        component: null,
        query: '',
        isFrame: '0',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'guide',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: 'nest-admin-soybean官网地址',
        delFlag: 'NORMAL',
      },
      {
        menuId: 5,
        tenantId: '000000',
        menuName: '门店管理',
        parentId: 0,
        orderNum: 5,
        path: 'store',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '门店管理目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 200,
        tenantId: '000000',
        menuName: '财务',
        parentId: 5,
        orderNum: 1,
        path: 'finance',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'money',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '门店财务目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 201,
        tenantId: '000000',
        menuName: '佣金明细',
        parentId: 200,
        orderNum: 1,
        path: 'commission',
        component: 'store/finance/commission/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:finance:commission:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '佣金明细菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 202,
        tenantId: '000000',
        menuName: '财务仪表盘',
        parentId: 200,
        orderNum: 2,
        path: 'dashboard',
        component: 'store/finance/dashboard/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:finance:dashboard:list',
        icon: 'dashboard',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '财务仪表盘菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 203,
        tenantId: '000000',
        menuName: '分销配置',
        parentId: 200,
        orderNum: 3,
        path: 'distribution-config',
        component: 'store/finance/distribution-config/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:finance:distribution-config:list',
        icon: 'edit',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销配置菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 204,
        tenantId: '000000',
        menuName: '流水明细',
        parentId: 200,
        orderNum: 4,
        path: 'ledger',
        component: 'store/finance/ledger/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:finance:ledger:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '流水明细菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 205,
        tenantId: '000000',
        menuName: '提现管理',
        parentId: 200,
        orderNum: 5,
        path: 'withdrawal',
        component: 'store/finance/withdrawal/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:finance:withdrawal:list',
        icon: 'money',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '提现管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 210,
        tenantId: '000000',
        menuName: '订单',
        parentId: 5,
        orderNum: 2,
        path: 'order',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '门店订单目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 211,
        tenantId: '000000',
        menuName: '订单详情',
        parentId: 210,
        orderNum: 1,
        path: 'detail',
        component: 'store/order/detail/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '1',
        status: 'NORMAL',
        perms: 'store:order:detail',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '订单详情（隐藏）',
        delFlag: 'NORMAL',
      },
      {
        menuId: 212,
        tenantId: '000000',
        menuName: '订单列表',
        parentId: 210,
        orderNum: 2,
        path: 'list',
        component: 'store/order/list/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:order:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '订单列表菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 213,
        tenantId: '000000',
        menuName: '待派单',
        parentId: 210,
        orderNum: 3,
        path: 'dispatch',
        component: 'store/order/dispatch/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:order:dispatch:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '订单待派单菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 220,
        tenantId: '000000',
        menuName: '分销',
        parentId: 5,
        orderNum: 3,
        path: 'distribution',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'tree',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销管理目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 221,
        tenantId: '000000',
        menuName: '分销活动',
        parentId: 220,
        orderNum: 1,
        path: 'activity',
        component: 'store/distribution/activity/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:activity:list',
        icon: 'date',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销活动菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 222,
        tenantId: '000000',
        menuName: '优惠券发放',
        parentId: 220,
        orderNum: 2,
        path: 'coupon-distribution',
        component: 'store/distribution/coupon-distribution/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:coupon:list',
        icon: 'documentation',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '优惠券发放菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 223,
        tenantId: '000000',
        menuName: '优惠券使用',
        parentId: 220,
        orderNum: 3,
        path: 'coupon-usage',
        component: 'store/distribution/coupon-usage/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:coupon-usage:list',
        icon: 'documentation',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '优惠券使用菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 224,
        tenantId: '000000',
        menuName: '分销员',
        parentId: 220,
        orderNum: 4,
        path: 'distribution',
        component: 'store/distribution/distribution/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:list',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销员菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 225,
        tenantId: '000000',
        menuName: '分销申请',
        parentId: 220,
        orderNum: 5,
        path: 'distribution-application',
        component: 'store/distribution/distribution-application/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:application:list',
        icon: 'form',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销申请菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 226,
        tenantId: '000000',
        menuName: '分销仪表盘',
        parentId: 220,
        orderNum: 6,
        path: 'distribution-dashboard',
        component: 'store/distribution/distribution-dashboard/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:dashboard:list',
        icon: 'dashboard',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销仪表盘菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 227,
        tenantId: '000000',
        menuName: '分销等级',
        parentId: 220,
        orderNum: 7,
        path: 'distribution-level',
        component: 'store/distribution/distribution-level/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:level:list',
        icon: 'star',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销等级菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 228,
        tenantId: '000000',
        menuName: '分销商品',
        parentId: 220,
        orderNum: 8,
        path: 'distribution-product',
        component: 'store/distribution/distribution-product/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:distribution:product:list',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '分销商品菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 230,
        tenantId: '000000',
        menuName: '商品',
        parentId: 5,
        orderNum: 4,
        path: 'product',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '门店商品目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 231,
        tenantId: '000000',
        menuName: '商品列表',
        parentId: 230,
        orderNum: 1,
        path: 'list',
        component: 'store/product/list/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:product:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品列表菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 232,
        tenantId: '000000',
        menuName: '商品市场',
        parentId: 230,
        orderNum: 2,
        path: 'market',
        component: 'store/product/market/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:product:market:list',
        icon: 'guide',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品市场菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 240,
        tenantId: '000000',
        menuName: '库存',
        parentId: 5,
        orderNum: 5,
        path: 'stock',
        component: 'store/stock/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'store:stock:list',
        icon: 'tree-table',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '库存菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 6,
        tenantId: '000000',
        menuName: '会员管理',
        parentId: 0,
        orderNum: 6,
        path: 'member',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '会员管理目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 250,
        tenantId: '000000',
        menuName: '会员列表',
        parentId: 6,
        orderNum: 1,
        path: 'list',
        component: 'member/list/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'member:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '会员列表菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 251,
        tenantId: '000000',
        menuName: '升级申请',
        parentId: 6,
        orderNum: 2,
        path: 'upgrade',
        component: 'member/upgrade/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'member:upgrade:list',
        icon: 'star',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '会员升级申请菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 7,
        tenantId: '000000',
        menuName: '营销管理',
        parentId: 0,
        orderNum: 7,
        path: 'marketing',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销管理目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 260,
        tenantId: '000000',
        menuName: '营销资产',
        parentId: 7,
        orderNum: 1,
        path: 'asset',
        component: 'marketing/asset/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:asset:list',
        icon: 'money',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销资产菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 261,
        tenantId: '000000',
        menuName: '优惠券',
        parentId: 7,
        orderNum: 2,
        path: 'coupon',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'documentation',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '优惠券目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 262,
        tenantId: '000000',
        menuName: '优惠券模板',
        parentId: 261,
        orderNum: 1,
        path: 'template',
        component: 'marketing/coupon/template/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:coupon:template:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '优惠券模板菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 263,
        tenantId: '000000',
        menuName: '积分',
        parentId: 7,
        orderNum: 3,
        path: 'points',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'star',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '积分目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 264,
        tenantId: '000000',
        menuName: '积分账户',
        parentId: 263,
        orderNum: 1,
        path: 'accounts',
        component: 'marketing/points/accounts/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:points:accounts:list',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '积分账户菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 265,
        tenantId: '000000',
        menuName: '积分规则',
        parentId: 263,
        orderNum: 2,
        path: 'rules',
        component: 'marketing/points/rules/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:points:rules:list',
        icon: 'documentation',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '积分规则菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 266,
        tenantId: '000000',
        menuName: '积分统计',
        parentId: 263,
        orderNum: 3,
        path: 'statistics',
        component: 'marketing/points/statistics/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:points:statistics:list',
        icon: 'chart',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '积分统计菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 267,
        tenantId: '000000',
        menuName: '积分任务',
        parentId: 263,
        orderNum: 4,
        path: 'tasks',
        component: 'marketing/points/tasks/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:points:tasks:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '积分任务菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 268,
        tenantId: '000000',
        menuName: '营销模板',
        parentId: 7,
        orderNum: 4,
        path: 'template',
        component: 'marketing/template/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:template:list',
        icon: 'documentation',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销模板菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 269,
        tenantId: '000000',
        menuName: '营销统计',
        parentId: 7,
        orderNum: 5,
        path: 'statistics',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'chart',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '营销统计目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 270,
        tenantId: '000000',
        menuName: '优惠券统计',
        parentId: 269,
        orderNum: 1,
        path: 'coupon',
        component: 'marketing/statistics/coupon/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:statistics:coupon:list',
        icon: 'chart',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '优惠券统计菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 271,
        tenantId: '000000',
        menuName: '课程管理',
        parentId: 7,
        orderNum: 6,
        path: 'course-management',
        component: 'marketing/course-management/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:course:list',
        icon: 'education',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '课程管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 272,
        tenantId: '000000',
        menuName: '课程排期',
        parentId: 7,
        orderNum: 7,
        path: 'course-schedule',
        component: 'marketing/course-schedule/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:course:schedule:list',
        icon: 'date',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '课程排期菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 273,
        tenantId: '000000',
        menuName: '课程考勤',
        parentId: 7,
        orderNum: 8,
        path: 'course-attendance',
        component: 'marketing/course-attendance/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'marketing:course:attendance:list',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '课程考勤菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 8,
        tenantId: '000000',
        menuName: '商品中心',
        parentId: 0,
        orderNum: 8,
        path: 'pms',
        component: null,
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品中心目录',
        delFlag: 'NORMAL',
      },
      {
        menuId: 280,
        tenantId: '000000',
        menuName: '商品属性',
        parentId: 8,
        orderNum: 1,
        path: 'attribute',
        component: 'pms/attribute/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'pms:attribute:list',
        icon: 'list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品属性菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 281,
        tenantId: '000000',
        menuName: '商品品牌',
        parentId: 8,
        orderNum: 2,
        path: 'brand',
        component: 'pms/brand/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'pms:brand:list',
        icon: 'star',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品品牌菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 282,
        tenantId: '000000',
        menuName: '商品分类',
        parentId: 8,
        orderNum: 3,
        path: 'category',
        component: 'pms/category/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'pms:category:list',
        icon: 'tree',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '商品分类菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 283,
        tenantId: '000000',
        menuName: '商品管理',
        parentId: 8,
        orderNum: 4,
        path: 'global-product',
        component: 'pms/global-product/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'pms:product:list',
        icon: 'shopping',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '全局商品管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 284,
        tenantId: '000000',
        menuName: '新建商品',
        parentId: 8,
        orderNum: 5,
        path: 'global-product-create',
        component: 'pms/global-product-create/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '1',
        status: 'NORMAL',
        perms: 'pms:product:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '新建商品（隐藏）',
        delFlag: 'NORMAL',
      },
      {
        menuId: 121,
        tenantId: '000000',
        menuName: '客户端管理',
        parentId: 1,
        orderNum: 12,
        path: 'client',
        component: 'system/client/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:client:list',
        icon: 'client',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: 'OAuth 客户端管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 122,
        tenantId: '000000',
        menuName: '消息管理',
        parentId: 1,
        orderNum: 13,
        path: 'message',
        component: 'system/message/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:message:list',
        icon: 'message',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '消息管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 123,
        tenantId: '000000',
        menuName: '对象存储',
        parentId: 1,
        orderNum: 14,
        path: 'oss',
        component: 'system/oss/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:oss:list',
        icon: 'upload',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '对象存储菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 124,
        tenantId: '000000',
        menuName: 'OSS 配置',
        parentId: 1,
        orderNum: 15,
        path: 'oss-config',
        component: 'system/oss-config/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '1',
        status: 'NORMAL',
        perms: 'system:ossConfig:list',
        icon: 'config',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: 'OSS 配置（隐藏）',
        delFlag: 'NORMAL',
      },
      {
        menuId: 125,
        tenantId: '000000',
        menuName: '任务日志',
        parentId: 2,
        orderNum: 7,
        path: 'job-log',
        component: 'monitor/job-log/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:jobLog:list',
        icon: 'log',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '定时任务日志菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 100,
        tenantId: '000000',
        menuName: '用户管理',
        parentId: 1,
        orderNum: 1,
        path: 'user',
        component: 'system/user/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:list',
        icon: 'user',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '用户管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 101,
        tenantId: '000000',
        menuName: '角色管理',
        parentId: 1,
        orderNum: 2,
        path: 'role',
        component: 'system/role/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:list',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '角色管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 102,
        tenantId: '000000',
        menuName: '菜单管理',
        parentId: 1,
        orderNum: 3,
        path: 'menu',
        component: 'system/menu/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:menu:list',
        icon: 'tree-table',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '菜单管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 103,
        tenantId: '000000',
        menuName: '部门管理',
        parentId: 1,
        orderNum: 4,
        path: 'dept',
        component: 'system/dept/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dept:list',
        icon: 'tree',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '部门管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 104,
        tenantId: '000000',
        menuName: '岗位管理',
        parentId: 1,
        orderNum: 5,
        path: 'post',
        component: 'system/post/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:list',
        icon: 'post',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '岗位管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 105,
        tenantId: '000000',
        menuName: '字典管理',
        parentId: 1,
        orderNum: 6,
        path: 'dict',
        component: 'system/dict/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:list',
        icon: 'dict',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '字典管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 106,
        tenantId: '000000',
        menuName: '参数设置',
        parentId: 1,
        orderNum: 7,
        path: 'config',
        component: 'system/config/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:list',
        icon: 'edit',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '参数设置菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 107,
        tenantId: '000000',
        menuName: '通知公告',
        parentId: 1,
        orderNum: 8,
        path: 'notice',
        component: 'system/notice/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:notice:list',
        icon: 'message',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '通知公告菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 118,
        tenantId: '000000',
        menuName: '租户管理',
        parentId: 1,
        orderNum: 9,
        path: 'tenant',
        component: 'system/tenant/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:list',
        icon: 'peoples',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '租户管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 119,
        tenantId: '000000',
        menuName: '租户套餐管理',
        parentId: 1,
        orderNum: 10,
        path: 'tenant-package',
        component: 'system/tenant-package/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:list',
        icon: 'skill',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '租户套餐管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 120,
        tenantId: '000000',
        menuName: '文件管理',
        parentId: 1,
        orderNum: 11,
        path: 'file-manager',
        component: 'system/file-manager/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:list',
        icon: 'folder',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '文件管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 108,
        tenantId: '000000',
        menuName: '日志管理',
        parentId: 1,
        orderNum: 9,
        path: 'log',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'M',
        visible: '0',
        status: 'NORMAL',
        perms: '',
        icon: 'log',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '日志管理菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 109,
        tenantId: '000000',
        menuName: '在线用户',
        parentId: 2,
        orderNum: 1,
        path: 'online',
        component: 'monitor/online/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:online:list',
        icon: 'online',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '在线用户菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 110,
        tenantId: '000000',
        menuName: '定时任务',
        parentId: 2,
        orderNum: 2,
        path: 'job',
        component: 'monitor/job/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:list',
        icon: 'job',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '定时任务菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 111,
        tenantId: '000000',
        menuName: '数据监控',
        parentId: 2,
        orderNum: 3,
        path: 'druid',
        component: 'monitor/druid/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:druid:list',
        icon: 'druid',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '数据监控菜单',
        delFlag: 'DELETE',
      },
      {
        menuId: 112,
        tenantId: '000000',
        menuName: '服务监控',
        parentId: 2,
        orderNum: 4,
        path: 'server',
        component: 'monitor/server/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:server:list',
        icon: 'server',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '服务监控菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 113,
        tenantId: '000000',
        menuName: '缓存监控',
        parentId: 2,
        orderNum: 5,
        path: 'cache',
        component: 'monitor/cache/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:cache:list',
        icon: 'redis',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '缓存监控菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 114,
        tenantId: '000000',
        menuName: '缓存列表',
        parentId: 2,
        orderNum: 6,
        path: 'cacheList',
        component: 'monitor/cache/list',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:cache:list',
        icon: 'redis-list',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '缓存列表菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 115,
        tenantId: '000000',
        menuName: '表单构建',
        parentId: 3,
        orderNum: 1,
        path: 'build',
        component: 'tool/build/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:build:list',
        icon: 'build',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '表单构建菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 116,
        tenantId: '000000',
        menuName: '代码生成',
        parentId: 3,
        orderNum: 2,
        path: 'gen',
        component: 'tool/gen/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:list',
        icon: 'code',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '代码生成菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 117,
        tenantId: '000000',
        menuName: '系统接口',
        parentId: 3,
        orderNum: 3,
        path: 'swagger',
        component: 'tool/swagger/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:swagger:list',
        icon: 'swagger',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '系统接口菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 500,
        tenantId: '000000',
        menuName: '操作日志',
        parentId: 108,
        orderNum: 1,
        path: 'operlog',
        component: 'monitor/operlog/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:operlog:list',
        icon: 'form',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '操作日志菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 501,
        tenantId: '000000',
        menuName: '登录日志',
        parentId: 108,
        orderNum: 2,
        path: 'logininfor',
        component: 'monitor/logininfor/index',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'C',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:logininfor:list',
        icon: 'logininfor',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '登录日志菜单',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1000,
        tenantId: '000000',
        menuName: '用户查询',
        parentId: 100,
        orderNum: 1,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1001,
        tenantId: '000000',
        menuName: '用户新增',
        parentId: 100,
        orderNum: 2,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1002,
        tenantId: '000000',
        menuName: '用户修改',
        parentId: 100,
        orderNum: 3,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1003,
        tenantId: '000000',
        menuName: '用户删除',
        parentId: 100,
        orderNum: 4,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1004,
        tenantId: '000000',
        menuName: '用户导出',
        parentId: 100,
        orderNum: 5,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1005,
        tenantId: '000000',
        menuName: '用户导入',
        parentId: 100,
        orderNum: 6,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:import',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1006,
        tenantId: '000000',
        menuName: '重置密码',
        parentId: 100,
        orderNum: 7,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:user:resetPwd',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1007,
        tenantId: '000000',
        menuName: '角色查询',
        parentId: 101,
        orderNum: 1,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1008,
        tenantId: '000000',
        menuName: '角色新增',
        parentId: 101,
        orderNum: 2,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1009,
        tenantId: '000000',
        menuName: '角色修改',
        parentId: 101,
        orderNum: 3,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1010,
        tenantId: '000000',
        menuName: '角色删除',
        parentId: 101,
        orderNum: 4,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1011,
        tenantId: '000000',
        menuName: '角色导出',
        parentId: 101,
        orderNum: 5,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:role:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1012,
        tenantId: '000000',
        menuName: '菜单查询',
        parentId: 102,
        orderNum: 1,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:menu:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1013,
        tenantId: '000000',
        menuName: '菜单新增',
        parentId: 102,
        orderNum: 2,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:menu:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1014,
        tenantId: '000000',
        menuName: '菜单修改',
        parentId: 102,
        orderNum: 3,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:menu:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1015,
        tenantId: '000000',
        menuName: '菜单删除',
        parentId: 102,
        orderNum: 4,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:menu:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1016,
        tenantId: '000000',
        menuName: '部门查询',
        parentId: 103,
        orderNum: 1,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dept:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1017,
        tenantId: '000000',
        menuName: '部门新增',
        parentId: 103,
        orderNum: 2,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dept:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1018,
        tenantId: '000000',
        menuName: '部门修改',
        parentId: 103,
        orderNum: 3,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dept:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1019,
        tenantId: '000000',
        menuName: '部门删除',
        parentId: 103,
        orderNum: 4,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dept:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1020,
        tenantId: '000000',
        menuName: '岗位查询',
        parentId: 104,
        orderNum: 1,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1021,
        tenantId: '000000',
        menuName: '岗位新增',
        parentId: 104,
        orderNum: 2,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1022,
        tenantId: '000000',
        menuName: '岗位修改',
        parentId: 104,
        orderNum: 3,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1023,
        tenantId: '000000',
        menuName: '岗位删除',
        parentId: 104,
        orderNum: 4,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1024,
        tenantId: '000000',
        menuName: '岗位导出',
        parentId: 104,
        orderNum: 5,
        path: '',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:post:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1025,
        tenantId: '000000',
        menuName: '字典查询',
        parentId: 105,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1026,
        tenantId: '000000',
        menuName: '字典新增',
        parentId: 105,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1027,
        tenantId: '000000',
        menuName: '字典修改',
        parentId: 105,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1028,
        tenantId: '000000',
        menuName: '字典删除',
        parentId: 105,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1029,
        tenantId: '000000',
        menuName: '字典导出',
        parentId: 105,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:dict:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1030,
        tenantId: '000000',
        menuName: '参数查询',
        parentId: 106,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1031,
        tenantId: '000000',
        menuName: '参数新增',
        parentId: 106,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1032,
        tenantId: '000000',
        menuName: '参数修改',
        parentId: 106,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1033,
        tenantId: '000000',
        menuName: '参数删除',
        parentId: 106,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1034,
        tenantId: '000000',
        menuName: '参数导出',
        parentId: 106,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:config:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1035,
        tenantId: '000000',
        menuName: '公告查询',
        parentId: 107,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:notice:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1036,
        tenantId: '000000',
        menuName: '公告新增',
        parentId: 107,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:notice:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1037,
        tenantId: '000000',
        menuName: '公告修改',
        parentId: 107,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:notice:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1038,
        tenantId: '000000',
        menuName: '公告删除',
        parentId: 107,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:notice:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1039,
        tenantId: '000000',
        menuName: '操作查询',
        parentId: 500,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:operlog:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1040,
        tenantId: '000000',
        menuName: '操作删除',
        parentId: 500,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:operlog:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1041,
        tenantId: '000000',
        menuName: '日志导出',
        parentId: 500,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:operlog:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1042,
        tenantId: '000000',
        menuName: '登录查询',
        parentId: 501,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:logininfor:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1043,
        tenantId: '000000',
        menuName: '登录删除',
        parentId: 501,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:logininfor:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1044,
        tenantId: '000000',
        menuName: '日志导出',
        parentId: 501,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:logininfor:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1045,
        tenantId: '000000',
        menuName: '账户解锁',
        parentId: 501,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:logininfor:unlock',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1046,
        tenantId: '000000',
        menuName: '在线查询',
        parentId: 109,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:online:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1047,
        tenantId: '000000',
        menuName: '批量强退',
        parentId: 109,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:online:batchLogout',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1048,
        tenantId: '000000',
        menuName: '单条强退',
        parentId: 109,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:online:forceLogout',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1049,
        tenantId: '000000',
        menuName: '任务查询',
        parentId: 110,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1050,
        tenantId: '000000',
        menuName: '任务新增',
        parentId: 110,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1051,
        tenantId: '000000',
        menuName: '任务修改',
        parentId: 110,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1052,
        tenantId: '000000',
        menuName: '任务删除',
        parentId: 110,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1053,
        tenantId: '000000',
        menuName: '状态修改',
        parentId: 110,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:changeStatus',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1054,
        tenantId: '000000',
        menuName: '任务导出',
        parentId: 110,
        orderNum: 6,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'monitor:job:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1055,
        tenantId: '000000',
        menuName: '生成查询',
        parentId: 116,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1056,
        tenantId: '000000',
        menuName: '生成修改',
        parentId: 116,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1057,
        tenantId: '000000',
        menuName: '生成删除',
        parentId: 116,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1058,
        tenantId: '000000',
        menuName: '导入代码',
        parentId: 116,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:import',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1059,
        tenantId: '000000',
        menuName: '预览代码',
        parentId: 116,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:preview',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1060,
        tenantId: '000000',
        menuName: '生成代码',
        parentId: 116,
        orderNum: 6,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'tool:gen:code',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1061,
        tenantId: '000000',
        menuName: '租户查询',
        parentId: 118,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1062,
        tenantId: '000000',
        menuName: '租户新增',
        parentId: 118,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1063,
        tenantId: '000000',
        menuName: '租户修改',
        parentId: 118,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1064,
        tenantId: '000000',
        menuName: '租户删除',
        parentId: 118,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1065,
        tenantId: '000000',
        menuName: '租户导出',
        parentId: 118,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenant:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1066,
        tenantId: '000000',
        menuName: '租户套餐查询',
        parentId: 119,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1067,
        tenantId: '000000',
        menuName: '租户套餐新增',
        parentId: 119,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1068,
        tenantId: '000000',
        menuName: '租户套餐修改',
        parentId: 119,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1069,
        tenantId: '000000',
        menuName: '租户套餐删除',
        parentId: 119,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1070,
        tenantId: '000000',
        menuName: '租户套餐导出',
        parentId: 119,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:tenantPackage:export',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1071,
        tenantId: '000000',
        menuName: '文件查询',
        parentId: 120,
        orderNum: 1,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:query',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1072,
        tenantId: '000000',
        menuName: '文件新增',
        parentId: 120,
        orderNum: 2,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:add',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1073,
        tenantId: '000000',
        menuName: '文件编辑',
        parentId: 120,
        orderNum: 3,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:edit',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1074,
        tenantId: '000000',
        menuName: '文件删除',
        parentId: 120,
        orderNum: 4,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1075,
        tenantId: '000000',
        menuName: '文件分享',
        parentId: 120,
        orderNum: 5,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:share',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1076,
        tenantId: '000000',
        menuName: '回收站列表',
        parentId: 120,
        orderNum: 6,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:recycle:list',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1077,
        tenantId: '000000',
        menuName: '回收站恢复',
        parentId: 120,
        orderNum: 7,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:recycle:restore',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
      {
        menuId: 1078,
        tenantId: '000000',
        menuName: '回收站删除',
        parentId: 120,
        orderNum: 8,
        path: '#',
        component: '',
        query: '',
        isFrame: '1',
        isCache: '0',
        menuType: 'F',
        visible: '0',
        status: 'NORMAL',
        perms: 'system:file:recycle:remove',
        icon: '#',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
        delFlag: 'NORMAL',
      },
    ],
    skipDuplicates: true,
  });

  // sys_user
  await prisma.sysUser.createMany({
    data: [
      {
        userId: 1,
        tenantId: '000000',
        deptId: 103,
        userName: 'admin',
        nickName: 'Nest Admin',
        userType: '00',
        email: 'admin@nestadmin.com',
        phonenumber: '15888888888',
        sex: '1',
        avatar: '',
        password: '$2b$10$UrJrjy0kxyrTO1UvhRVsvex35mB1s1jzAraIA9xtzPmlLmRtZXEXS',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        loginIp: '127.0.0.1',
        loginDate: null,
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '管理员',
      },
      {
        userId: 2,
        tenantId: '000000',
        deptId: 105,
        userName: 'test',
        nickName: 'Nest Admin Test',
        userType: '00',
        email: 'test@nestadmin.com',
        phonenumber: '15666666666',
        sex: '1',
        avatar: '',
        password: '$2b$10$UrJrjy0kxyrTO1UvhRVsvex35mB1s1jzAraIA9xtzPmlLmRtZXEXS',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        loginIp: '127.0.0.1',
        loginDate: null,
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '测试员',
      },
      {
        userId: 3,
        tenantId: '000000',
        deptId: 103,
        userName: 'demo',
        nickName: '演示账号',
        userType: '00',
        email: 'demo@example.com',
        phonenumber: '13800138000',
        sex: '0',
        avatar: '',
        password: '$2b$10$g3kM8fAzWz4LAb9bgBJAruofmfFL13xUw1QqOTdrLvouZCLbY7sVa',
        status: 'NORMAL',
        delFlag: 'NORMAL',
        loginIp: '::1',
        loginDate: new Date('2025-12-23 11:08:09.236'),
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '演示账户，密码：demo123',
      },
    ],
    skipDuplicates: true,
  });

  // sys_notice
  await prisma.sysNotice.createMany({
    data: [
      {
        noticeId: 1,
        tenantId: '000000',
        noticeTitle: '温馨提醒：2025-01-01 Nest-Admin-Soybean新版本发布啦',
        noticeType: '2',
        noticeContent: '新版本内容',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        delFlag: 'NORMAL',
        remark: null,
      },
      {
        noticeId: 2,
        tenantId: '000000',
        noticeTitle: '维护通知：2025-01-01 Nest-Admin-Soybean系统凌晨维护',
        noticeType: '1',
        noticeContent: '维护内容',
        status: 'NORMAL',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        delFlag: 'NORMAL',
        remark: null,
      },
    ],
    skipDuplicates: true,
  });

  // sys_job
  await prisma.sysJob.createMany({
    data: [
      {
        jobId: 1,
        tenantId: '000000',
        jobName: '系统默认（无参）',
        jobGroup: 'DEFAULT',
        invokeTarget: 'task.noParams',
        cronExpression: '0/10 * * * * ?',
        misfirePolicy: '3',
        concurrent: '1',
        status: 'STOP',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
      },
      {
        jobId: 2,
        tenantId: '000000',
        jobName: '系统默认（有参）',
        jobGroup: 'DEFAULT',
        invokeTarget: 'task.params(\'ry\')',
        cronExpression: '0/15 * * * * ?',
        misfirePolicy: '3',
        concurrent: '1',
        status: 'STOP',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
      },
      {
        jobId: 3,
        tenantId: '000000',
        jobName: '系统默认（多参）',
        jobGroup: 'DEFAULT',
        invokeTarget: 'task.multipleParams(\'ry\', true, 2000L, 316.50D, 100)',
        cronExpression: '0/20 * * * * ?',
        misfirePolicy: '3',
        concurrent: '1',
        status: 'STOP',
        createBy: 'admin',
        createTime: new Date('2025-02-28 08:52:10'),
        updateBy: '',
        updateTime: null,
        remark: '',
      },
    ],
    skipDuplicates: true,
  });

  // sys_logininfor
  await prisma.sysLogininfor.createMany({
    data: [
      {
        infoId: 1,
        tenantId: '000000',
        userName: '',
        ipaddr: '::1',
        loginLocation: '',
        browser: 'Chrome 143.0.0',
        os: 'Mac OS X',
        deviceType: '0',
        status: 'NORMAL',
        msg: '登录成功',
        delFlag: 'NORMAL',
        loginTime: new Date('2025-12-23 19:08:09.250011'),
      },
    ],
    skipDuplicates: true,
  });

  // sys_role_dept
  await prisma.sysRoleDept.createMany({
    data: [
      {
        roleId: 2,
        deptId: 100,
      },
      {
        roleId: 2,
        deptId: 101,
      },
      {
        roleId: 2,
        deptId: 105,
      },
    ],
    skipDuplicates: true,
  });

  // sys_role_menu
  await prisma.sysRoleMenu.createMany({
    data: [
      {
        roleId: 2,
        menuId: 1,
      },
      {
        roleId: 2,
        menuId: 2,
      },
      {
        roleId: 2,
        menuId: 3,
      },
      {
        roleId: 2,
        menuId: 4,
      },
      {
        roleId: 2,
        menuId: 100,
      },
      {
        roleId: 2,
        menuId: 101,
      },
      {
        roleId: 2,
        menuId: 102,
      },
      {
        roleId: 2,
        menuId: 103,
      },
      {
        roleId: 2,
        menuId: 104,
      },
      {
        roleId: 2,
        menuId: 105,
      },
      {
        roleId: 2,
        menuId: 106,
      },
      {
        roleId: 2,
        menuId: 107,
      },
      {
        roleId: 2,
        menuId: 108,
      },
      {
        roleId: 2,
        menuId: 109,
      },
      {
        roleId: 2,
        menuId: 110,
      },
      {
        roleId: 2,
        menuId: 111,
      },
      {
        roleId: 2,
        menuId: 112,
      },
      {
        roleId: 2,
        menuId: 113,
      },
      {
        roleId: 2,
        menuId: 114,
      },
      {
        roleId: 2,
        menuId: 115,
      },
      {
        roleId: 2,
        menuId: 116,
      },
      {
        roleId: 2,
        menuId: 117,
      },
      {
        roleId: 2,
        menuId: 500,
      },
      {
        roleId: 2,
        menuId: 501,
      },
      {
        roleId: 2,
        menuId: 1000,
      },
      {
        roleId: 2,
        menuId: 1001,
      },
      {
        roleId: 2,
        menuId: 1002,
      },
      {
        roleId: 2,
        menuId: 1003,
      },
      {
        roleId: 2,
        menuId: 1004,
      },
      {
        roleId: 2,
        menuId: 1005,
      },
      {
        roleId: 2,
        menuId: 1006,
      },
      {
        roleId: 2,
        menuId: 1007,
      },
      {
        roleId: 2,
        menuId: 1008,
      },
      {
        roleId: 2,
        menuId: 1009,
      },
      {
        roleId: 2,
        menuId: 1010,
      },
      {
        roleId: 2,
        menuId: 1011,
      },
      {
        roleId: 2,
        menuId: 1012,
      },
      {
        roleId: 2,
        menuId: 1013,
      },
      {
        roleId: 2,
        menuId: 1014,
      },
      {
        roleId: 2,
        menuId: 1015,
      },
      {
        roleId: 2,
        menuId: 1016,
      },
      {
        roleId: 2,
        menuId: 1017,
      },
      {
        roleId: 2,
        menuId: 1018,
      },
      {
        roleId: 2,
        menuId: 1019,
      },
      {
        roleId: 2,
        menuId: 1020,
      },
      {
        roleId: 2,
        menuId: 1021,
      },
      {
        roleId: 2,
        menuId: 1022,
      },
      {
        roleId: 2,
        menuId: 1023,
      },
      {
        roleId: 2,
        menuId: 1024,
      },
      {
        roleId: 2,
        menuId: 1025,
      },
      {
        roleId: 2,
        menuId: 1026,
      },
      {
        roleId: 2,
        menuId: 1027,
      },
      {
        roleId: 2,
        menuId: 1028,
      },
      {
        roleId: 2,
        menuId: 1029,
      },
      {
        roleId: 2,
        menuId: 1030,
      },
      {
        roleId: 2,
        menuId: 1031,
      },
      {
        roleId: 2,
        menuId: 1032,
      },
      {
        roleId: 2,
        menuId: 1033,
      },
      {
        roleId: 2,
        menuId: 1034,
      },
      {
        roleId: 2,
        menuId: 1035,
      },
      {
        roleId: 2,
        menuId: 1036,
      },
      {
        roleId: 2,
        menuId: 1037,
      },
      {
        roleId: 2,
        menuId: 1038,
      },
      {
        roleId: 2,
        menuId: 1039,
      },
      {
        roleId: 2,
        menuId: 1040,
      },
      {
        roleId: 2,
        menuId: 1041,
      },
      {
        roleId: 2,
        menuId: 1042,
      },
      {
        roleId: 2,
        menuId: 1043,
      },
      {
        roleId: 2,
        menuId: 1044,
      },
      {
        roleId: 2,
        menuId: 1045,
      },
      {
        roleId: 2,
        menuId: 1046,
      },
      {
        roleId: 2,
        menuId: 1047,
      },
      {
        roleId: 2,
        menuId: 1048,
      },
      {
        roleId: 2,
        menuId: 1049,
      },
      {
        roleId: 2,
        menuId: 1050,
      },
      {
        roleId: 2,
        menuId: 1051,
      },
      {
        roleId: 2,
        menuId: 1052,
      },
      {
        roleId: 2,
        menuId: 1053,
      },
      {
        roleId: 2,
        menuId: 1054,
      },
      {
        roleId: 2,
        menuId: 1055,
      },
      {
        roleId: 2,
        menuId: 1056,
      },
      {
        roleId: 2,
        menuId: 1057,
      },
      {
        roleId: 2,
        menuId: 1058,
      },
      {
        roleId: 2,
        menuId: 1059,
      },
      {
        roleId: 2,
        menuId: 1060,
      },
      {
        roleId: 2,
        menuId: 118,
      },
      {
        roleId: 2,
        menuId: 119,
      },
      {
        roleId: 2,
        menuId: 1061,
      },
      {
        roleId: 2,
        menuId: 1062,
      },
      {
        roleId: 2,
        menuId: 1063,
      },
      {
        roleId: 2,
        menuId: 1064,
      },
      {
        roleId: 2,
        menuId: 1065,
      },
      {
        roleId: 2,
        menuId: 1066,
      },
      {
        roleId: 2,
        menuId: 1067,
      },
      {
        roleId: 2,
        menuId: 1068,
      },
      {
        roleId: 2,
        menuId: 1069,
      },
      {
        roleId: 2,
        menuId: 1070,
      },
      {
        roleId: 2,
        menuId: 120,
      },
      {
        roleId: 2,
        menuId: 1071,
      },
      {
        roleId: 2,
        menuId: 1072,
      },
      {
        roleId: 2,
        menuId: 1073,
      },
      {
        roleId: 2,
        menuId: 1074,
      },
      {
        roleId: 2,
        menuId: 1075,
      },
      {
        roleId: 2,
        menuId: 1076,
      },
      {
        roleId: 2,
        menuId: 1077,
      },
      {
        roleId: 2,
        menuId: 1078,
      },
      {
        roleId: 2,
        menuId: 5,
      },
      {
        roleId: 2,
        menuId: 200,
      },
      {
        roleId: 2,
        menuId: 201,
      },
      {
        roleId: 2,
        menuId: 202,
      },
      {
        roleId: 2,
        menuId: 203,
      },
      {
        roleId: 2,
        menuId: 204,
      },
      {
        roleId: 2,
        menuId: 205,
      },
      {
        roleId: 2,
        menuId: 210,
      },
      {
        roleId: 2,
        menuId: 211,
      },
      {
        roleId: 2,
        menuId: 212,
      },
      {
        roleId: 2,
        menuId: 220,
      },
      {
        roleId: 2,
        menuId: 221,
      },
      {
        roleId: 2,
        menuId: 222,
      },
      {
        roleId: 2,
        menuId: 223,
      },
      {
        roleId: 2,
        menuId: 224,
      },
      {
        roleId: 2,
        menuId: 225,
      },
      {
        roleId: 2,
        menuId: 226,
      },
      {
        roleId: 2,
        menuId: 227,
      },
      {
        roleId: 2,
        menuId: 228,
      },
      {
        roleId: 2,
        menuId: 230,
      },
      {
        roleId: 2,
        menuId: 231,
      },
      {
        roleId: 2,
        menuId: 232,
      },
      {
        roleId: 2,
        menuId: 240,
      },
      {
        roleId: 2,
        menuId: 213,
      },
      {
        roleId: 2,
        menuId: 6,
      },
      {
        roleId: 2,
        menuId: 250,
      },
      {
        roleId: 2,
        menuId: 251,
      },
      {
        roleId: 2,
        menuId: 7,
      },
      {
        roleId: 2,
        menuId: 260,
      },
      {
        roleId: 2,
        menuId: 261,
      },
      {
        roleId: 2,
        menuId: 262,
      },
      {
        roleId: 2,
        menuId: 263,
      },
      {
        roleId: 2,
        menuId: 264,
      },
      {
        roleId: 2,
        menuId: 265,
      },
      {
        roleId: 2,
        menuId: 266,
      },
      {
        roleId: 2,
        menuId: 267,
      },
      {
        roleId: 2,
        menuId: 268,
      },
      {
        roleId: 2,
        menuId: 269,
      },
      {
        roleId: 2,
        menuId: 270,
      },
      {
        roleId: 2,
        menuId: 271,
      },
      {
        roleId: 2,
        menuId: 272,
      },
      {
        roleId: 2,
        menuId: 273,
      },
      {
        roleId: 2,
        menuId: 8,
      },
      {
        roleId: 2,
        menuId: 280,
      },
      {
        roleId: 2,
        menuId: 281,
      },
      {
        roleId: 2,
        menuId: 282,
      },
      {
        roleId: 2,
        menuId: 283,
      },
      {
        roleId: 2,
        menuId: 284,
      },
      {
        roleId: 2,
        menuId: 121,
      },
      {
        roleId: 2,
        menuId: 122,
      },
      {
        roleId: 2,
        menuId: 123,
      },
      {
        roleId: 2,
        menuId: 124,
      },
      {
        roleId: 2,
        menuId: 125,
      },
      {
        roleId: 3,
        menuId: 1,
      },
      {
        roleId: 3,
        menuId: 2,
      },
      {
        roleId: 3,
        menuId: 3,
      },
      {
        roleId: 3,
        menuId: 4,
      },
      {
        roleId: 3,
        menuId: 108,
      },
      {
        roleId: 3,
        menuId: 100,
      },
      {
        roleId: 3,
        menuId: 101,
      },
      {
        roleId: 3,
        menuId: 102,
      },
      {
        roleId: 3,
        menuId: 103,
      },
      {
        roleId: 3,
        menuId: 104,
      },
      {
        roleId: 3,
        menuId: 105,
      },
      {
        roleId: 3,
        menuId: 106,
      },
      {
        roleId: 3,
        menuId: 107,
      },
      {
        roleId: 3,
        menuId: 118,
      },
      {
        roleId: 3,
        menuId: 119,
      },
      {
        roleId: 3,
        menuId: 120,
      },
      {
        roleId: 3,
        menuId: 109,
      },
      {
        roleId: 3,
        menuId: 110,
      },
      {
        roleId: 3,
        menuId: 112,
      },
      {
        roleId: 3,
        menuId: 113,
      },
      {
        roleId: 3,
        menuId: 114,
      },
      {
        roleId: 3,
        menuId: 115,
      },
      {
        roleId: 3,
        menuId: 116,
      },
      {
        roleId: 3,
        menuId: 117,
      },
      {
        roleId: 3,
        menuId: 500,
      },
      {
        roleId: 3,
        menuId: 501,
      },
      {
        roleId: 3,
        menuId: 1000,
      },
      {
        roleId: 3,
        menuId: 1004,
      },
      {
        roleId: 3,
        menuId: 1007,
      },
      {
        roleId: 3,
        menuId: 1011,
      },
      {
        roleId: 3,
        menuId: 1012,
      },
      {
        roleId: 3,
        menuId: 1016,
      },
      {
        roleId: 3,
        menuId: 1020,
      },
      {
        roleId: 3,
        menuId: 1024,
      },
      {
        roleId: 3,
        menuId: 1025,
      },
      {
        roleId: 3,
        menuId: 1029,
      },
      {
        roleId: 3,
        menuId: 1030,
      },
      {
        roleId: 3,
        menuId: 1034,
      },
      {
        roleId: 3,
        menuId: 1035,
      },
      {
        roleId: 3,
        menuId: 1039,
      },
      {
        roleId: 3,
        menuId: 1041,
      },
      {
        roleId: 3,
        menuId: 1042,
      },
      {
        roleId: 3,
        menuId: 1044,
      },
      {
        roleId: 3,
        menuId: 1046,
      },
      {
        roleId: 3,
        menuId: 1049,
      },
      {
        roleId: 3,
        menuId: 1054,
      },
      {
        roleId: 3,
        menuId: 1055,
      },
      {
        roleId: 3,
        menuId: 1059,
      },
      {
        roleId: 3,
        menuId: 1061,
      },
      {
        roleId: 3,
        menuId: 1065,
      },
      {
        roleId: 3,
        menuId: 1066,
      },
      {
        roleId: 3,
        menuId: 1070,
      },
      {
        roleId: 3,
        menuId: 1071,
      },
      {
        roleId: 3,
        menuId: 1076,
      },
      {
        roleId: 3,
        menuId: 8,
      },
      {
        roleId: 3,
        menuId: 280,
      },
      {
        roleId: 3,
        menuId: 281,
      },
      {
        roleId: 3,
        menuId: 282,
      },
      {
        roleId: 3,
        menuId: 283,
      },
      {
        roleId: 3,
        menuId: 284,
      },
      {
        roleId: 3,
        menuId: 121,
      },
      {
        roleId: 3,
        menuId: 122,
      },
      {
        roleId: 3,
        menuId: 123,
      },
      {
        roleId: 3,
        menuId: 124,
      },
      {
        roleId: 3,
        menuId: 125,
      },
    ],
    skipDuplicates: true,
  });

  // sys_user_post
  await prisma.sysUserPost.createMany({
    data: [
      {
        userId: 1,
        postId: 1,
      },
      {
        userId: 2,
        postId: 2,
      },
      {
        userId: 3,
        postId: 1,
      },
    ],
    skipDuplicates: true,
  });

  // sys_user_role
  await prisma.sysUserRole.createMany({
    data: [
      {
        userId: 1,
        roleId: 1,
      },
      {
        userId: 2,
        roleId: 2,
      },
      {
        userId: 3,
        roleId: 3,
      },
    ],
    skipDuplicates: true,
  });

  // 业务种子：总部 → 租户 → 选品 → C端（分类/分佣/商品/营销/会员等）
  const { runSeedPhases } = await import('./seeds/run-phases');
  await runSeedPhases(prisma);

  // 重置 Postgres 序列
  console.log('正在重置数据库序列...');
  const tableNames = [
    'sys_user', 'sys_role', 'sys_menu', 'sys_dept', 'sys_post',
    'sys_dict_type', 'sys_dict_data', 'sys_config', 'sys_notice',
    'sys_job', 'sys_job_log', 'sys_oper_log', 'sys_logininfor',
    'sys_tenant', 'sys_tenant_package', 'sys_client',
    'gen_table', 'gen_table_column', 'sys_file_folder',
    'pms_category', 'sys_dist_config', 'sys_dist_level',
  ];

  for (const tableName of tableNames) {
    try {
      // 获取表的主键列名 (通常是 id, user_id 等)
      //这里简化处理，假设 Prisma 能够正确推断，或者我们直接尝试重置 explicit id columns
      // For more robustness, we could query information_schema, but strict list is safer for now if we know them.
      // But looking at schema, primary keys have different names (user_id, menu_id...).
      // Let's use a generic dynamic query to find the serial column.

      interface ColumnInfo {
        column_name: string;
        column_default: string | null;
      }
      const constraints = await prisma.$queryRawUnsafe<ColumnInfo[]>(
        `SELECT column_name, column_default 
         FROM information_schema.columns 
         WHERE table_name = '${tableName}' 
         AND column_default LIKE 'nextval%'`
      );

      if (constraints.length > 0) {
        const columnName = constraints[0].column_name;
        await prisma.$executeRawUnsafe(
          `SELECT setval(pg_get_serial_sequence('${tableName}', '${columnName}'), (SELECT COALESCE(MAX("${columnName}"), 1) FROM "${tableName}"))`
        );
        // console.log(`Reset sequence for ${tableName}.${columnName}`);
      }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error);
      console.warn(`Failed to reset sequence for ${tableName}:`, msg);
    }
  }
  console.log('数据库序列重置完成');

  console.log('种子数据导入完成!');
}

main()
  .catch((e) => {
    console.error('种子数据导入失败:', e);
    process.exit(1);
  })
  .finally(async () => {
    await prisma.$disconnect();
  });
