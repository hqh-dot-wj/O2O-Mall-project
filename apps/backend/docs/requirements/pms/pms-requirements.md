...
| AC-9  | SKU 创建时包含 `costPrice`（成本价）                            | ✅ 已通过（T-1） |
| AC-10 | 全局商品下架时通知已导入门店自动下架                            | ✅ 已通过（T-2, T-3） |
| AC-11 | 门店导入时校验 `distRate` 在 `minDistRate`~`maxDistRate` 范围内 | ✅ 已通过（T-9） |
| AC-12 | 商品支持删除（含级联处理）                                      | ✅ 已通过（T-8） |
...
| 编号 | 任务                                                    | 对应差距 | 预估工时 | 状态 |
| ---- | ------------------------------------------------------- | -------- | -------- | ---- |
| T-8  | 商品删除接口（含门店级联处理）                          | D-1, X-3 | 2-3d     | ✅ 短期已完成 |
| T-9  | 门店导入时校验 `distRate` 在 `min~max` 范围内           | X-5      | 1d       | ✅ 短期已完成 |
| T-10 | 商品变更事件机制（EventEmitter / Bull）                 | A-1      | 2-3d     | —    |
| T-11 | 商品导入/导出（Excel）                                  | A-3      | 2-3d     | —    |
| T-12 | 品牌名称唯一性校验                                      | A-7      | 0.5h     | ✅ 已完成 |
| T-13 | 分类层级自动计算                                        | A-6      | 1h       | ✅ 已完成 |
| T-14 | `PmsModule` 导出 `ProductRepository` 和 `SkuRepository` | X-4      | 0.5h     | ✅ 已完成 |
