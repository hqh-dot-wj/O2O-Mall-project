# 平台抽成与多租户架构说明

## 📋 目录

1. [平台抽成概念](#平台抽成概念)
2. [超级管理员租户 vs 普通租户](#超级管理员租户-vs-普通租户)
3. [资金流向](#资金流向)
4. [三种业务模式](#三种业务模式)
5. [实现建议](#实现建议)

---

## 💰 平台抽成概念

### 什么是平台抽成？

平台抽成是指**平台方（系统运营商）从每笔交易中收取的服务费**。

### 当前系统中的角色

在你的多租户SaaS系统中：

| 角色          | 租户ID                | 说明                       |
| ------------- | --------------------- | -------------------------- |
| **平台方**    | `000000`              | 超级管理员租户，系统运营商 |
| **门店/商家** | `100001`, `100002`... | 普通租户，使用系统的商家   |
| **消费者**    | -                     | C端用户，在门店消费        |

### 资金流向示例

```
消费者支付 ¥100
    ↓
┌─────────────────────────────────┐
│  订单总收入 ¥100                │
├─────────────────────────────────┤
│  ├─ 平台抽成（5%）：¥5          │  → 平台方（租户000000）
│  └─ 门店收入（95%）：¥95        │  → 门店（租户100001）
│     ├─ 分佣支出：¥10            │  → 推荐人
│     └─ 门店净利润：¥85          │  → 门店最终收入
└─────────────────────────────────┘
```

---

## 🏢 超级管理员租户 vs 普通租户

### 超级管理员租户（`000000`）

**定义位置：** `apps/backend/src/common/tenant/tenant.context.ts`

```typescript
static readonly SUPER_TENANT_ID = '000000';
```

**特权：**

1. ✅ 可以查看所有租户的数据
2. ✅ 可以管理所有租户
3. ✅ 可以切换到任意租户视角
4. ✅ 不受租户隔离限制
5. ✅ 可以配置全局系统参数

**用途：**

- 系统运营商
- 平台管理员
- 数据分析和监控
- 全局配置管理

**收入来源：**

- 租户订阅费（月费/年费）
- 交易抽成（如果启用）
- 增值服务费

### 普通租户（`100001`, `100002`...）

**特点：**

1. ❌ 只能查看自己的数据
2. ❌ 不能访问其他租户数据
3. ✅ 数据完全隔离
4. ✅ 独立的用户体系
5. ✅ 独立的商品和订单

**用途：**

- 门店/商家
- 独立运营
- 服务自己的客户

**收入来源：**

- 订单收入（扣除平台抽成后）
- 会员费
- 增值服务

---

## 💸 资金流向

### 完整的资金流向图

```
┌─────────────────────────────────────────────────────────────┐
│                    消费者支付 ¥4,030                         │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    订单总收入 ¥4,030                         │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  第一层分配：平台 vs 门店                                    │
│  ├─ 平台抽成（10%）：¥403                                    │
│  │  └─ 进入平台账户（租户000000）                           │
│  │                                                            │
│  └─ 门店毛收入（90%）：¥3,627                               │
│     └─ 进入门店账户（租户100001）                           │
│                                                               │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│              门店收入的二次分配 ¥3,627                       │
├─────────────────────────────────────────────────────────────┤
│                                                               │
│  第二层分配：分佣 vs 门店净利润                              │
│  ├─ 分佣支出：¥442                                           │
│  │  ├─ 张三（一级+二级）：¥238                              │
│  │  ├─ 李四（一级）：¥136                                   │
│  │  └─ 王五（一级）：¥68                                    │
│  │                                                            │
│  └─ 门店净利润：¥3,185                                       │
│     └─ 门店最终收入                                          │
│                                                               │
└─────────────────────────────────────────────────────────────┘
```

### 各方收入汇总

| 角色                 | 收入   | 占比 | 说明               |
| -------------------- | ------ | ---- | ------------------ |
| **平台方（000000）** | ¥403   | 10%  | 平台抽成           |
| **门店（100001）**   | ¥3,185 | 79%  | 扣除分佣后的净利润 |
| **推荐人（张三）**   | ¥238   | 5.9% | 一级+二级分佣      |
| **推荐人（李四）**   | ¥136   | 3.4% | 一级分佣           |
| **推荐人（王五）**   | ¥68    | 1.7% | 一级分佣           |
| **总计**             | ¥4,030 | 100% | -                  |

---

## 🎯 三种业务模式

### 模式1：纯SaaS工具（推荐）

**特点：**

- ❌ 无交易抽成
- ✅ 订阅费收入
- ✅ 门店获得100%订单收入

**收入模型：**

```
平台收入 = 租户数 × 订阅费
门店收入 = 订单收入 - 分佣支出
```

**优点：**

- 门店利润最大化
- 吸引更多商家入驻
- 商家满意度高

**缺点：**

- 平台收入依赖订阅
- 需要持续提供价值

**适用场景：**

- 工具型SaaS
- 企业服务
- 私域流量运营

**例子：**

- Shopify（基础版）
- 有赞（标准版）
- 微盟

---

### 模式2：SaaS + 交易抽成（混合）

**特点：**

- ✅ 低比例抽成（3-10%）
- ✅ 订阅费 + 交易抽成
- ✅ 双重收入来源

**收入模型：**

```
平台收入 = (租户数 × 订阅费) + (总交易额 × 抽成比例)
门店收入 = (订单收入 × (1 - 抽成比例)) - 分佣支出
```

**优点：**

- 平台收入更稳定
- 与商家共同成长
- 激励平台提供更好服务

**缺点：**

- 需要提供支付通道
- 资金结算复杂
- 需要财务合规

**适用场景：**

- 交易型SaaS
- 电商平台
- 本地生活服务

**例子：**

- 美团（3-5%）
- 饿了么（5-8%）
- 微信支付（0.6%）

---

### 模式3：纯平台模式

**特点：**

- ✅ 高比例抽成（20-30%）
- ❌ 无订阅费或低订阅费
- ✅ 提供流量和用户

**收入模型：**

```
平台收入 = 总交易额 × 抽成比例
门店收入 = (订单收入 × (1 - 抽成比例)) - 分佣支出
```

**优点：**

- 平台收入与GMV直接挂钩
- 激励平台带来更多流量
- 规模效应明显

**缺点：**

- 商家利润压力大
- 需要强大的流量能力
- 竞争激烈

**适用场景：**

- 电商平台
- 外卖平台
- 到店服务平台

**例子：**

- 淘宝（5-10%）
- 京东（8-15%）
- 抖音小店（2-10%）

---

## 🛠️ 实现建议

### 1. 配置化抽成比例

在 `SysTenant` 表中添加字段：

```prisma
model SysTenant {
  // ... 现有字段

  // 平台抽成配置
  commissionRate  Decimal @default(0.00) @map("commission_rate") @db.Decimal(5, 2) // 抽成比例（%）
  commissionMode  String  @default("NONE") @map("commission_mode") // NONE, FIXED, TIERED

  // 结算配置
  settlementCycle Int     @default(7) @map("settlement_cycle") // 结算周期（天）
  minWithdraw     Decimal @default(100.00) @map("min_withdraw") @db.Decimal(10, 2) // 最低提现金额
}
```

### 2. 创建平台收入表

```prisma
model FinPlatformIncome {
  id          BigInt   @id @default(autoincrement())
  orderId     String   @map("order_id")
  tenantId    String   @map("tenant_id") // 来源租户

  orderAmount Decimal  @db.Decimal(10, 2) // 订单金额
  rate        Decimal  @db.Decimal(5, 2)  // 抽成比例
  amount      Decimal  @db.Decimal(10, 2) // 抽成金额

  status      String   @default("PENDING") // PENDING, SETTLED
  settleTime  DateTime? @map("settle_time")

  createTime  DateTime @default(now()) @map("create_time")

  @@index([tenantId, status])
  @@map("fin_platform_income")
}
```

### 3. 订单创建时计算抽成

```typescript
async function createOrder(orderData: CreateOrderDto) {
  // 1. 获取租户抽成配置
  const tenant = await prisma.sysTenant.findUnique({
    where: { tenantId: orderData.tenantId },
  });

  const commissionRate = Number(tenant.commissionRate) / 100;

  // 2. 计算金额分配
  const orderAmount = orderData.totalAmount;
  const platformCommission = orderAmount * commissionRate;
  const storeRevenue = orderAmount - platformCommission;

  // 3. 创建订单
  const order = await prisma.omsOrder.create({
    data: {
      ...orderData,
      platformCommission, // 新增字段
      storeRevenue, // 新增字段
    },
  });

  // 4. 如果有抽成，创建平台收入记录
  if (platformCommission > 0) {
    await prisma.finPlatformIncome.create({
      data: {
        orderId: order.id,
        tenantId: orderData.tenantId,
        orderAmount,
        rate: tenant.commissionRate,
        amount: platformCommission,
        status: 'PENDING',
      },
    });
  }

  return order;
}
```

### 4. 结算流程

```typescript
// 定时任务：每天结算
async function settlePlatformIncome() {
  const pendingIncomes = await prisma.finPlatformIncome.findMany({
    where: {
      status: 'PENDING',
      createTime: {
        lte: new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // 7天前
      },
    },
  });

  for (const income of pendingIncomes) {
    await prisma.finPlatformIncome.update({
      where: { id: income.id },
      data: {
        status: 'SETTLED',
        settleTime: new Date(),
      },
    });
  }
}
```

---

## 📊 数据查询示例

### 查询平台总收入

```typescript
// 超级管理员（000000）查询所有租户的平台收入
const platformIncome = await prisma.finPlatformIncome.aggregate({
  _sum: {
    amount: true,
  },
  where: {
    status: 'SETTLED',
    createTime: {
      gte: startDate,
      lte: endDate,
    },
  },
});

console.log(`平台总收入：¥${platformIncome._sum.amount}`);
```

### 查询门店净收入

```typescript
// 普通租户（100001）查询自己的净收入
const storeIncome = await prisma.omsOrder.aggregate({
  _sum: {
    storeRevenue: true, // 扣除平台抽成后的收入
  },
  where: {
    tenantId: '100001',
    payStatus: 'PAID',
    createTime: {
      gte: startDate,
      lte: endDate,
    },
  },
});

// 减去分佣支出
const commissions = await prisma.finCommission.aggregate({
  _sum: {
    amount: true,
  },
  where: {
    tenantId: '100001',
    status: 'SETTLED',
  },
});

const netIncome = Number(storeIncome._sum.storeRevenue) - Number(commissions._sum.amount);
console.log(`门店净收入：¥${netIncome}`);
```

---

## 🎯 推荐方案

根据你的系统特点，我推荐：

### 阶段1：纯SaaS模式（当前）

- ❌ 暂不启用平台抽成
- ✅ 专注于功能完善
- ✅ 吸引商家入驻
- ✅ 积累用户和数据

### 阶段2：混合模式（成熟后）

- ✅ 启用低比例抽成（3-5%）
- ✅ 提供支付通道
- ✅ 提供营销工具
- ✅ 提供数据分析

### 阶段3：平台模式（规模化）

- ✅ 根据GMV调整抽成
- ✅ 提供流量支持
- ✅ 建立生态体系

---

## 📝 总结

1. **平台抽成** = 平台方从每笔交易中收取的服务费
2. **超级管理员租户（000000）** = 平台方，可以查看所有数据
3. **普通租户（100001...）** = 门店/商家，数据隔离
4. **资金流向**：消费者 → 平台抽成 + 门店收入 → 分佣 + 门店净利润
5. **建议**：从纯SaaS模式开始，逐步过渡到混合模式

---

## 相关文档

- [端到端测试指南](./E2E_TEST_GUIDE.md)
- [分佣配置说明](./COMMISSION_CONFIG.md)
- [多租户架构](./MULTI_TENANT_ARCHITECTURE.md)
