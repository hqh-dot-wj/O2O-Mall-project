...
- [ ] T-1: 所有 Controller 添加 `@ApiBearerAuth` + `@RequirePermission` — 覆盖 coupon(10) + points(15) + maas(21) 共 46+ 端点 (4h)
- [ ] T-2: 所有写操作添加 `@Operlog` — 覆盖全部 POST/PUT/PATCH/DELETE 端点 (2h)
- [ ] T-3: 所有 Controller 使用 `@Api` 替代 `@ApiOperation` — points(15) + C 端(6) 共 21 端点 (2h)
- [ ] T-4: 所有 Service 统一使用已定义的 ErrorCode 替代硬编码字符串 — coupon(20 个错误码) + points(23 个错误码) (4h)
...
- [ ] T-5: 参与活动流程接入库存扣减 — STRONG_LOCK 模式当前形同虚设 (3h) — maas D-10
- [ ] T-6: batchTransitStatus 增加状态机校验 + 事件发送 — 当前跳过 isValidTransition 和分布式锁 (3h) — maas D-7/D-8
- [ ] T-7: 互斥矩阵 SECKILL → FLASH_SALE 命名修正 — 秒杀活动互斥检查永远不生效 (0.5h) — maas D-4

### 积分系统修复

- [ ] T-8: freezePoints / unfreezePoints 添加乐观锁 — 并发场景余额可能不一致 (3h) — points D-11
- [ ] T-9: completeTask 添加 @Transactional 事务包裹 — 发放成功但记录失败时可重复领奖 (1h) — points D-14

### 订单集成修复

- [ ] T-10: 5 个 handle\* 方法添加幂等性保护（Redis SetNX） — 网络重试导致重复处理 (4h) — integration D-7
- [ ] T-11: handleOrderPaid 积分扣减（解冻+扣减）添加事务包裹 — 解冻成功但扣减失败时状态不一致 (1h) — integration D-4
- [ ] T-12: 退款时消费积分扣减添加余额校验 — 用户已消费积分时扣减会失败 (1h) — integration D-6

## 阶段三：基础设施补完 + 架构规范对齐（2-3 周）

### 基础设施补完

- [ ] T-13: 所有定时任务添加分布式锁（Redis SET NX） — coupon/points/maas/infra 共 4 处调度器 (4h) — infra D-5, coupon D-5, points D-9, maas D-12
- [ ] T-14: 审批流完整实现 — 当前所有方法均为 TODO 示例代码，连接数据库 (1.5d) — infra D-12
- [ ] T-15: 缓存装饰器实现 AOP 拦截器 — @Cacheable/@CacheEvict 当前仅定义元数据不生效 (1.5d) — infra D-19
- [ ] T-16: 实现事件监听器 — 当前 10 种事件类型有发送无消费者，至少覆盖 SUCCESS/FAILED/TIMEOUT (2d) — maas A-7, infra X-1

### 跨模块访问治理

- [ ] T-17: 消除对 omsOrder 表的直接访问 — coupon(2 处) + integration(3 处) 共 5 处，改用 OrderService (1d) — coupon D-10/D-11, integration D-2/D-3
- [ ] T-18: 消除对 umsMember 表的直接访问 — coupon(1 处) + points(1 处) 共 2 处，改用 MemberService (0.5d) — coupon X-3, points X-1

### 规范对齐

- [ ] T-19: C 端 Controller 装饰器规范化 — coupon/points/integration C 端共 7 个 Controller，统一 @Api + @ApiBearerAuth + @tenantScope (0.5d) — coupon X-2, points X-2, integration D-1
- [ ] T-20: 定时任务 catch 使用 getErrorMessage 安全提取 + 分批处理 — coupon D-6, points D-10 (0.5d)
- [ ] T-21: MaaS creditToStore 资产类型从 rules.giftAssetType 读取，平台费率默认值写入配置表 (0.5d) — maas D-5/D-6
- [ ] T-22: MaaS 删除废弃的 checkTransition 方法 + 修复归档任务空 data (0.5h) — maas D-13/D-14
- [ ] T-23: Integration 5 个 handle\* 方法添加分布式锁 (2h) — integration D-8
- [ ] T-24: Integration 错误日志使用 getErrorStack 记录堆栈 (0.5h) — integration D-9

## 阶段四：性能优化 + 运营能力增强（1-2 月）

### 性能优化

- [ ] T-25: coupon 7 日趋势改为单次聚合查询（消除 14 次 DB 查询的 N+1） (0.5d) — coupon D-7
- [ ] T-26: points 签到连续天数改为单次查询 + 内存计算（消除最多 100 次 DB 查询） (0.5d) — points D-6
- [ ] T-27: points 任务完成记录改为 WHERE IN 批量查询 (0.5d) — points D-7
- [ ] T-28: coupon + points 导出接口统一添加数量限制（最大 10000 条或异步导出） (1d) — coupon D-8, points D-8
- [ ] T-29: coupon 手动发放添加批量上限（如 500 人） (0.5h) — coupon D-9

### 事件驱动统一改造

- [ ] T-30: coupon 添加事件驱动（领取/使用/过期事件） (2d) — coupon A-7
- [ ] T-31: points 添加事件驱动（获取/使用/过期事件） (2d) — points X-5
- [ ] T-32: integration 5 个关键节点发送事件到 marketing/events (2d) — integration X-4
- [ ] T-33: 明确 coupon/points 与 integration 的对接契约文档化 (1d) — coupon X-4, points X-3
