...
describe('ProfitValidator', () => {
  ...
  describe('validate', () => {
    it('应该通过验证 - 利润充足', () => {
      expect(() => {
        validator.validate(100, new Decimal(50), 10, DistributionMode.PERCENTAGE);
      }).not.toThrow();
    });

    it('应该抛出异常 - 价格低于成本价', () => {
      expect(() => {
        validator.validate(50, new Decimal(100), 10, DistributionMode.PERCENTAGE);
      }).toThrow(BusinessException);
    });

    it('应该抛出异常 - 利润不足以支付佣金', () => {
      expect(() => {
        validator.validate(100, new Decimal(95), 10, DistributionMode.PERCENTAGE);
      }).toThrow(BusinessException);
    });
  ...
  describe('calculateProfit', () => {
    it('应该正确计算利润 - 百分比模式', () => {
      const result = validator.calculateProfit(100, new Decimal(50), 10, DistributionMode.PERCENTAGE);
    ...
    it('应该返回无效结果 - 价格低于成本', () => {
      const result = validator.calculateProfit(50, new Decimal(100), 10, DistributionMode.PERCENTAGE);

      expect(result.isValid).toBe(false);
    });

    it('应该返回无效结果 - 利润不足', () => {
      const result = validator.calculateProfit(100, new Decimal(95), 10, DistributionMode.PERCENTAGE);

      expect(result.isValid).toBe(false);
    });

    it('应该处理边界情况 - 利润刚好等于佣金', () => {
      const result = validator.calculateProfit(110, new Decimal(100), 10, DistributionMode.PERCENTAGE);
    ...
    it('应该处理零佣金', () => {
      const result = validator.calculateProfit(100, new Decimal(50), 0, DistributionMode.PERCENTAGE);
