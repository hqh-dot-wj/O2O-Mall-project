    ...
    <div class="recycle-bin-wrapper h-full flex flex-col">
        ...
        <div class="toolbar-container mb-4 p-4 bg-white rounded-lg">
            <n-space justify="space-between">
                <n-space>
                    <n-input v-model:value="searchParams.fileName" placeholder="搜索文件名" clearable
        ...
        <div class="flex-1 overflow-hidden">
            <n-data-table :columns="columns" :data="dataSource" :loading="loading" :pagination="pagination"
                :row-key="(row) => row.uploadId" :checked-row-keys="checkedRowKeys"
                @update:checked-row-keys="handleCheck" @update:page="handlePageChange"
                @update:page-size="handlePageSizeChange" class="h-full" flex-height remote />
        </div>
    </div>
</template>
        ...
        render: row => (
            <NSpace>
                <NButton
                    ...
                    type="primary"
                    onClick={() => handleRestore([row.uploadId])}
                >
                    恢复
                </NButton>
                <NButton
                    size={themeStore.componentSize}
                    type="error"
                    onClick={() => handleDelete([row.uploadId])}
                >
                    删除
                </NButton>
            </NSpace>
