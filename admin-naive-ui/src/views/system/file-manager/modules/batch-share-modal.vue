...
async function handleCreateShares() {
    ...
    try {
        ...
        const promises = uploadIds.value.map(uploadId =>
            fetchCreateShare({ ...params, uploadId })
        );
...
<template>
    <NModal v-model:show="visible" preset="card" title="批量分享" class="w-700px" @after-leave="handleClose">
        <div v-if="shareResults.length === 0">
            <NForm ref="formRef" :model="formModel" label-placement="left" label-width="120px">
                <NFormItem label="有效期（小时）" path="expireHours">
                    <NInputNumber v-model:value="formModel.expireHours" placeholder="-1 表示永久有效" class="w-full"
                        :min="-1" />
                </NFormItem>

                <NFormItem label="最大下载次数" path="maxDownload">
                    <NInputNumber v-model:value="formModel.maxDownload" placeholder="-1 表示不限制" class="w-full"
                        :min="-1" />
                </NFormItem>
            ...
            <div class="text-14px text-gray mb-4">
                将为 {{ uploadIds.length }} 个文件创建分享链接
            </div>

            <NSpace justify="end" class="mt-4">
                <NButton @click="handleClose">取消</NButton>
                <NButton type="primary" :loading="loading" @click="handleCreateShares">
                    批量创建分享
                </NButton>
            </NSpace>
        </div>
        ...
        <div v-else class="share-results">
            ...
            <NButton type="primary" block @click="handleClose" class="mt-4">
                完成
            </NButton>
        </div>
    </NModal>
