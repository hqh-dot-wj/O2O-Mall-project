<template>
    <div ref="containerRef" class="file-grid-view">
        <n-virtual-list v-if="rows.length > 0" :item-size="CARD_HEIGHT + CARD_GAP" :items="rows"
            :style="{ height: `${containerHeight}px` }">
            <template #default="{ item: row, index }">
                <div class="grid-row" :style="{ marginBottom: `${CARD_GAP}px` }">
                    <div v-for="(file, fileIndex) in row" :key="file.id" class="file-card-wrapper"
                        :style="cardWrapperStyle">
                        <n-tooltip :delay="500" placement="bottom">
                            <template #trigger>
                                <n-card
                                    :class="['file-card', { selected: checkedKeys.includes(file.id), 'drag-over': file.type === 'folder' && dragOverId === file.id }]"
                                    :hoverable="true" :style="cardStyle(checkedKeys.includes(file.id))"
                                    :draggable="file.type === 'file'" @click="handleCardClick(file, $event)"
                                    ...
                                    <div class="thumbnail-area">
                                        <n-image v-if="file.thumbnail" :src="file.thumbnail" :width="THUMBNAIL_WIDTH"
                                            :height="THUMBNAIL_HEIGHT" :lazy="true" :fallback-src="placeholderImage"
                                            object-fit="cover" :preview-disabled="true" />
...
.file-grid-view {
    ...
    .file-card {
        ...
        &:hover {
            box-shadow: 0 0 1px rgba(28, 28, 32, 0.05), 0 8px 24px rgba(28, 28, 32, 0.12);
        }
