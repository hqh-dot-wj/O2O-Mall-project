<template>
    <div class="sidebar-menu" :style="{ width: `${totalWidth}px` }">
        ...
        <div class="primary-menu" :style="{ width: `${SIDEBAR_PRIMARY_WIDTH}px` }">
            <div v-for="item in primaryMenuItems" :key="item.key" class="primary-menu-item"
                :class="{ active: activeMenu === item.key }" @click="handlePrimaryMenuClick(item.key)">
        ...
        <Transition name="slide">
            <div v-if="showSecondaryMenu" class="secondary-menu" :style="{ width: `${SIDEBAR_SECONDARY_WIDTH}px` }">
                <n-menu v-model:value="activeSecondaryMenu" :options="secondaryMenuOptions" :indent="MENU_INDENT"
                    :root-indent="12" @update:value="handleSecondaryMenuClick" />
            </div>
        </Transition>
    </div>
</template>
...
const totalWidth = computed(() => {
    return showSecondaryMenu.value
